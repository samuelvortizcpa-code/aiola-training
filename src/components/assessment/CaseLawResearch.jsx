...
export default function CaseLawResearch({ block, onComplete }) {
  ...
  return (
    <div style={{ background: B.card, border: `1px solid ${B.bdr}`, borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,.06)", overflow: "hidden" }}>
      <div style={{ padding: "12px 18px", borderBottom: `1px solid ${B.bdr}`, display: "flex", alignItems: "center", gap: 6 }}>
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><rect x="3" y="1" width="10" height="14" rx="1.5" stroke={B.blue} strokeWidth="1.3" fill="none" /><path d="M5.5 5h5M5.5 7.5h5M5.5 10h3" stroke={B.blue} strokeWidth="1.1" strokeLinecap="round" /></svg>
        <span style={{ fontSize: 12, fontWeight: 700, color: B.navy, textTransform: "uppercase", letterSpacing: .8 }}>Case Law Research</span>
      </div>

      <div style={{ padding: 18 }}>
        {/* Case header */}
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: B.navy, lineHeight: 1.4 }}>{block.case_name}</div>
          {block.citation && (
            <div style={{ fontSize: 11, color: B.t3, marginTop: 2 }}>{block.citation}</div>
          )}
        </div>
        ...
        {submitted && (
          <>
            <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: 8, background: B.blueL, border: `1px solid ${B.blueM}`, fontSize: 12, color: B.t1, lineHeight: 1.6 }}>
              <div style={{ fontWeight: 700, marginBottom: 8, fontSize: 11, color: B.blue }}>Did your brief cover these?</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {(block.evaluation_criteria || []).map((criterion, idx) => (
                  <label key={idx} style={{ display: "flex", alignItems: "flex-start", gap: 8, cursor: "pointer", fontSize: 12, color: B.t1 }}>
                    <input
                      type="checkbox"
                      checked={!!checkedCriteria[idx]}
                      onChange={() => toggleCriterion(idx)}
                      style={{ marginTop: 2, accentColor: B.blue }}
                    />
                    <span>{criterion}</span>
                  </label>
                ))}
